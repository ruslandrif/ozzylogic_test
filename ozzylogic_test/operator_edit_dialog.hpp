...
class operator_edit_dialog : public QDialog {
	Q_OBJECT
public:
	operator_edit_dialog(const std::string& name, const std::string& country_code, const std::vector<country>& recs, QWidget* parent) : QDialog(parent), records_(recs) {
	...
	void add_operator() { //add new operator
